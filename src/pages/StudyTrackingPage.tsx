...
const StudyTrackingPage: React.FC = () => {
    const { user } = useAuth();
    ...
    const canEdit = useMemo(() => {
        ...
        if (user.role === 'vice_monitor_study') return week >= currentRealWeek;
        return false;
    ...
    const loadData = async () => {
        ...
        try {
            const res = await api.get('/specialized', {
                params: { class_id: selectedClass.id, week, type: 'study' },
            });
    ...
    const STUDY_VIOLATIONS = [
        { code: 'NO_HOMEWORK', label: 'Không làm bài tập' },
        { code: 'NO_PREP', label: 'Không chuẩn bị bài' },
        { code: 'NO_LESSON', label: 'Không thuộc bài' },
        { code: 'SCORE_1_4', label: 'Điểm 1-4' },
        { code: 'SCORE_5_7', label: 'Điểm 5-7' },
        { code: 'SCORE_8_10', label: 'Điểm 8-10' },
    ];
    ...
    const handleDelete = async (ids: number[]) => {
        ...
        try {
            await api.delete(`/specialized/${ids.join(',')}`, { params: { type: 'study' } });
            loadData();
        } catch (e) {
            alert('Lỗi xóa');
        }
...
export default StudyTrackingPage;
