...
const ClassManagementPage: React.FC = () => {
  ...
  return (
    <div className="container" style={{padding: 20}}>
      ...
      <button className="btn-submit" onClick={() => setShowModal(true)}>+ Thêm Lớp Mới</button>

      <div className="class-grid" style={{display: 'flex', gap: 20, marginTop: 20, flexWrap: 'wrap'}}>
        {classes.map(cls => (
          <div key={cls.id} className="card" style={{padding: 20, border: '1px solid #ddd', width: 250, cursor: 'pointer'}}
               onClick={() => handleSelectClass(cls)}>
            <h2 style={{color: '#2196f3'}}>{cls.name}</h2>
            <p>Năm học: <b>{cls.school_year}</b></p>
            <div style={{marginTop: 10}}>
                <button 
                    style={{backgroundColor: 'red', color: 'white', border: 'none', padding: '5px 10px', borderRadius: 4}}
        ...
        <div className="modal-overlay" style={{position: 'fixed', top:0, left:0, right:0, bottom:0, background: 'rgba(0,0,0,0.5)', display:'flex', justifyContent:'center', alignItems:'center'}}>
          <div className="modal-content" style={{background: 'white', padding: 30, borderRadius: 8, width: 400}}>
             ...
             <div style={{display: 'flex', justifyContent: 'flex-end', gap: 10}}>
                ...
                <button className="btn-submit" onClick={handleCreateClass}>Lưu</button>
             </div>
          </div>
        </div>
      )}
